...
class User(db.Base):
    ...
    updated_at: Mapped[datetime] = Column(DateTime(timezone=True), server_default=func.now())
...
class Resource(db.Base):
    ...
    parent_id: Mapped[UUID] = Column(SQLAUUID(as_uuid=True), ForeignKey("lmdev_resources.id"))
    child_count: Mapped[int] = Column(Integer)
    slug: Mapped[str] = Column(String)
    author_id: Mapped[UUID] = Column(SQLAUUID(as_uuid=True), ForeignKey("user_profiles.id"))
